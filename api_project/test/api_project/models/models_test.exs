defmodule ApiProject.ModelsTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      ...
      assert clock.time == ~N[2010-04-17 14:00:00.000000]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      assert {:ok, clock} = Models.update_clock(clock, @update_attrs)
      assert %Clock{} = clock
      assert clock.status == false
      assert clock.time == ~N[2011-05-18 15:01:01.000000]
    end
  ...
  describe "workingtimes" do
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      assert {:ok, workingtime} = Models.update_workingtime(workingtime, @update_attrs)
  ...
  describe "clocks" do
    ...
    @valid_attrs %{status: true, time: "2010-04-17 14:00:00.000000Z"}
    ...
    test "create_clock/1 with valid data creates a clock" do
      assert {:ok, %Clock{} = clock} = Models.create_clock(@valid_attrs)
      assert clock.status == true
      assert clock.time == DateTime.from_naive!(~N[2010-04-17 14:00:00.000000Z], "Etc/UTC")
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      ...
      assert %Clock{} = clock
      assert clock.status == false
      assert clock.time == DateTime.from_naive!(~N[2011-05-18 15:01:01.000000Z], "Etc/UTC")
    end
  ...
  describe "users" do
    ...
    @valid_attrs %{email: "some email", password: "some password", token: "some token", username: "some username"}
    @update_attrs %{email: "some updated email", password: "some updated password", token: "some updated token", username: "some updated username"}
    @invalid_attrs %{email: nil, password: nil, token: nil, username: nil}
