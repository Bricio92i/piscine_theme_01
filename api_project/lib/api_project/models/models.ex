defmodule ApiProject.Models do
  @moduledoc """
  The Models context.
  """
  ...
  def get_user_by_email_pass!(email, password), do: Repo.get!(User, [email, password])
  ...
  def create_user(attrs \\ %{}) do
    %User{token: :crypto.strong_rand_bytes(32) |> Base.encode64 |> binary_part(0, 32)}
  ...
  def get_clock_by_userID!(user_id) do
    Repo.all(from(clock in Clock, where: clock.user_id == ^user_id))
  end
  ...
  def create_clock(user_id, attrs \\ %{}) do
    %Clock{user_id: String.to_integer(user_id)}
  ...
  def get_workingtime_by_userID_start_end!(user_id, wt_start, wt_end) do
    ...
    wt_nd = Timex.parse!(wt_end, "%Y-%m-%d %H:%M:%S", :strftime)
    Repo.all(from(worktime in Workingtime, where: worktime.user_id == ^user_id, where: worktime.start == ^wt_st, where: worktime.end == ^wt_nd))
  end

  ## get_workingtime by user_id and workingtime_id
  def get_workingtime_by_userID_wtID!(user_id, workingtime_id) do
    Repo.get_by(Workingtime, [user_id: user_id, id: workingtime_id])
  end
  ...
  def create_workingtime(user_id, attrs \\ %{}) do
    %Workingtime{user_id: String.to_integer(user_id)}
